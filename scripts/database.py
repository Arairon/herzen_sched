...
class Database:
    ...
    def get_user(self, user_id):
        with self.connection:
            user_data = self.connection.execute(
                "SELECT group_id, sub_group FROM users WHERE user_id = ?", (user_id,)
            ).fetchone()
        return user_data

    def set_mailing_time(self, user_id: int, mailing_time: str):
        with self.connection:
            self.connection.execute("UPDATE users SET mailing = ? WHERE user_id = ?", (mailing_time, user_id))

    def del_mailing_time(self, user_id: int):
        with self.connection:
            self.connection.execute("UPDATE users SET mailing = NULL WHERE user_id = ?", (user_id,))

    def get_mailing_time(self, user_id: int):
        with self.connection:
            mailing_time = self.connection.execute("SELECT mailing FROM users WHERE user_id = ?", (user_id,)).fetchone()
    ...
    def get_all_id(self):
        with self.connection:
            all_user_ids = self.connection.execute("SELECT user_id FROM users").fetchall()
        return all_user_ids
